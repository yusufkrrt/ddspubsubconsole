...
{
public:
    TopicDataType_MyType3()
    {
        set_name("MyType3");
        max_serialized_type_size = static_cast<uint32_t>(sizeof(MyType3));
    ...
    {
        const MyType3* mydata = static_cast<const MyType3*>(data);
        if (payload.max_size < sizeof(MyType3))
            return false;
        std::memcpy(payload.data, mydata, sizeof(MyType3));
        payload.length = static_cast<uint32_t>(sizeof(MyType3));
        return true;
    }

    bool deserialize(eprosima::fastdds::rtps::SerializedPayload_t& payload, void* data) override
    {
        if (payload.length < sizeof(MyType3))
            return false;
        std::memcpy(data, payload.data, sizeof(MyType3));
        return true;
    }

    uint32_t calculate_serialized_size(const void* /*data*/, eprosima::fastdds::dds::DataRepresentationId_t) override
    {
        return static_cast<uint32_t>(sizeof(MyType3));
    }

    void* create_data() override { return new MyType3(); }
    void delete_data(void* data) override { delete static_cast<MyType3*>(data); }
